...
public class BastionManager implements IBastionManager {
	...
	public void init() {
		...
		LinkedList <GroupManager.PlayerType> memberAndAbove = new LinkedList<GroupManager.PlayerType>();
	...
	private boolean hasBastionAccess(List<Player> players, Block block, ICitadel citadel) {
		...
		for(BastionBlock bastion : bastions) {
			...
			Reinforcement playerRein = rein != null && (rein instanceof Reinforcement) ? (Reinforcement)rein : null;
